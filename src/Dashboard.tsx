...
const ROLE_COLORS: Record<string, string> = {
    admin: "bg-amber-500/15 border-amber-400/30 text-amber-700 dark:text-amber-500",
    leader: "bg-indigo-500/15 border-indigo-400/30 text-indigo-700 dark:text-indigo-400",
    planning_lead: "bg-rose-500/15 border-rose-400/30 text-rose-700 dark:text-rose-400",
    musician: "bg-purple-500/15 border-purple-400/30 text-purple-700 dark:text-purple-400",
    audio_tech: "bg-teal-500/15 border-teal-400/30 text-teal-700 dark:text-teal-400",
    qa_specialist: "bg-fuchsia-500/15 border-fuchsia-400/30 text-fuchsia-700 dark:text-fuchsia-400",
    member: "bg-gray-500/15 border-gray-400/30 text-gray-600 dark:text-gray-400",
};
...
function RoleBadgeChip({ role }: { role: string }) {
    ...
    return (
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-sm font-bold border ${color}`}>
            {label}
        </div>
    );
}
...
function Chip({ name, photo }: { name: string; photo?: string }) {
    ...
    return (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 font-medium">
            {ok ? <img src={photo} className="w-6 h-6 rounded-full object-cover" alt={name} onError={() => setErr(true)} />
                : <div className="w-6 h-6 rounded-full bg-indigo-200 dark:bg-indigo-700 flex items-center justify-center text-[10px] font-bold text-indigo-700 dark:text-indigo-200">{init}</div>}
            {name}
        </div>
    );
}
    ...
    if (!schedule) return (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1">
                ...
                <p className="text-base text-gray-400 mt-0.5">Check with your leader or Planning Lead for assignments.</p>
            </div>
            <button onClick={() => onNavigate("schedule")} className="shrink-0 flex items-center gap-1.5 text-sm font-semibold px-3 py-2 rounded-xl bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 transition-colors">
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-indigo-200 dark:border-indigo-800/50 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-indigo-100 dark:border-indigo-800/30 bg-indigo-50/50 dark:bg-indigo-900/10">
                ...
                <button onClick={() => onNavigate("schedule")} className="text-sm text-indigo-500 hover:text-indigo-400 flex items-center gap-1 font-medium">
            ...
            <div className="p-5 space-y-4">
                ...
                <div className="flex items-start gap-3">
                    <div className="flex flex-col items-center justify-center w-14 py-2 rounded-xl bg-indigo-600 text-white shrink-0 shadow-md gap-0">
                        <p className="text-[11px] font-bold uppercase text-indigo-200">{new Date(schedule.date + "T00:00:00").toLocaleDateString("en", { month: "short" })}</p>
                        <p className="text-2xl font-black leading-tight">{new Date(schedule.date + "T00:00:00").getDate()}</p>
                        <p className="text-sm text-indigo-300">{new Date(schedule.date + "T00:00:00").toLocaleDateString("en", { weekday: "short" })}</p>
                    </div>
                    <div>
                        {/* H3 — Event name: one step below card H2 */}
                        <div className="flex items-start flex-wrap gap-x-2 gap-y-1">
                            <h3 className="text-base font-semibold text-gray-900 dark:text-white">{schedule.eventName ?? "Upcoming Event"}</h3>
                            <span className={`px-3 py-1.5 rounded-full text-sm font-bold shrink-0 ${du === "Today" ? "bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400" : du === "Tomorrow" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" : "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300"}`}>
                                {du}
                            </span>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap mt-0.5">
                            <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${svcColor(schedule.serviceType)}`}>{svcLabel(schedule.serviceType)}</span>
                            <span className="text-sm text-gray-400">{new Date(schedule.date + "T00:00:00").toLocaleDateString("en", { weekday: "long" })}</span>
                        </div>
                    </div>
                </div>
                ...
                {myRoles.length > 0 && (
                    <div>
                        <h4 className="text-sm font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 shrink-0">Your Role</h4>
                        <div className="flex flex-wrap gap-1.5">
                            {myRoles.map(r => (
                                <span key={r} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 text-sm font-semibold">
                                    <Mic2 size={10} /> {r}
                                </span>
                            ))}
                        </div>
                ...
                {(solemn || joyful) && (
                    <div>
                        <h4 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-1.5 flex items-center gap-1"><Music size={12} /> Songs to Prepare</h4>
                        <div className="flex flex-wrap gap-2">
                            {solemn && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30 rounded-xl text-sm">
                                    ...
                                    <span className="text-gray-400 text-sm">Solemn:</span>
                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{toSafeTitle(solemn.title)}</span>
                                </div>
                            )}
                            {joyful && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-violet-50 dark:bg-violet-900/20 border border-violet-100 dark:border-violet-800/30 rounded-xl text-sm">
                                    ...
                                    <span className="text-gray-400 text-sm">Joyful:</span>
                                    <span className="font-semibold text-gray-900 dark:text-gray-100">{toSafeTitle(joyful.title)}</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* Who else is serving */}
                <div>
                    <h4 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-1.5">Serving Together</h4>
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        {!schedule.worshipLeader && (schedule.musicians ?? []).length === 0 && (schedule.backupSingers ?? []).length === 0 && (schedule.assignments ?? []).length === 0 && (
                            <p className="text-sm text-gray-400">No other assignments yet</p>
                        )}
                    </div>
                </div>

                {schedule.notes && (
                    <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-700/40 text-sm text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700">
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                <h2 className="flex items-center gap-2 font-bold text-gray-900 dark:text-white text-base">
                    <Megaphone size={15} className="text-indigo-500" /> Team Announcements
                    {broadcasts.length > 0 && <span className="text-sm font-bold bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400 px-1.5 py-0.5 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse inline-block" /> {broadcasts.length} live</span>}
                </h2>
                {isAdmin && <button onClick={() => onNavigate("admin")} className="text-sm text-indigo-500 hover:text-indigo-400 flex items-center gap-1 font-medium">Manage <ChevronRight size={13} /></button>}
            </div>
            {loading ? (
                ...
                <div className="flex items-center gap-3 px-5 py-5 text-gray-400">
                    ...
                    <p className="text-base">No active announcements right now</p>
                </div>
            ) : (
                    ...
                    {broadcasts.slice(0, 3).map((b: any) => (
                        <div key={b.id} className="flex items-start gap-3 px-5 py-3.5">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-base text-gray-400 line-clamp-1 mt-0.5">{b.message}</p>
                            </div>
