        ...
        <div ref={panelRef} className="relative">
            ...
            {open && (
                ...
                >
                    ...
                    {/* \u2500\u2500 Filter Toolbar \u2500\u2500 */}
                    {!showForm && !showTrash && (
                        <div className="bg-white dark:bg-gray-800/30 border-b border-gray-200 dark:border-gray-700/60 shrink-0">

                            {/* Row 1 \u2014 Status Tabs */}
                            <div className="flex items-center gap-0.5 px-3 pt-2.5 pb-2">
                                {(["active", "resolved", "dismissed", "all"] as StatusTab[]).map(tab => {
                                    const count =
                                        tab === "active"    ? activeCount :
                                        tab === "resolved"  ? notes.filter(n => n.resolved).length :
                                        tab === "dismissed" ? dismissedCount :
                                        notes.length;
                                            ...
                                            {count > 0 && (
                                                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-none ${
                                                    isActive
                                                        ...
                                                        : "bg-gray-200 dark:bg-gray-600/50 text-gray-600 dark:text-gray-300"
                                                }`}>
                                                    {count}
                                                </span>
                                            )}
                            ...
                            {/* Subtle divider */}
                            <div className="mx-3 border-t border-gray-100 dark:border-gray-700/50" />

                            {/* Row 2 \u2014 Type filter (left) + Sort (right) */}
                            <div className="flex items-center gap-1.5 px-3 py-2">
                                ...
                                <div className="flex items-center gap-1 flex-1 overflow-x-auto no-scrollbar">
                                    {([
                                        ...
                                        const cnt = value === "all"
                                            ? filtered.length
                                            : filtered.filter(n => n.type === value).length;
                                        return (
                                            ...
                                            >
                                                ...
                                                {label}
                                                {cnt > 0 && (
                                                    <span className={`text-[10px] font-bold tabular-nums ${isActive ? "opacity-70" : "opacity-50"}`}>
                                                        {cnt}
                                                    </span>
                                                )}
                                            </button>
                                ...
                                <div ref={sortRef} className="relative shrink-0">
                                    ...
                                    {showSort && (
                                        <div className="absolute right-0 top-full mt-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden min-w-[140px]">
                                            {(Object.entries(SORT_LABELS) as [SortMode, string][]).map(([s, label]) => (
                                                ...
                                                >
                                                    {label}
                                                    {sort === s && <Check size={11} className="text-indigo-500" />}
                                                </button>
                                            ))}
                                        </div>
                                    )}
