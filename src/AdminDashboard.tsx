...
import {
    ...
    TrendingUp, ArrowUpRight, Star, Mic2, BookOpen, Radio, ExternalLink,
} from "lucide-react";
...
// ── Spotify Team Playlist Card ────────────────────────────────────────────────
const SPOTIFY_PLAYLIST_ID = "37i9dQZF1E4zvwJG7U2wF7";
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <div className="flex items-center gap-2 font-semibold text-gray-900 dark:text-white text-sm">
                            Team Playlist
                            <span className="flex items-center gap-1 text-[10px] font-bold bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400 px-1.5 py-0.5 rounded-full">
                                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse inline-block" />
                                Live
                            </span>
                        </div>
                        <p className="text-[11px] text-gray-400 mt-0.5">Worship reference tracks for the team</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <a
                        ...
                        className="flex items-center gap-1 text-xs text-[#1DB954] hover:text-[#17a349] font-semibold transition-colors"
                    >
                        Open Spotify <ExternalLink size={11} />
                    </a>
                    <button
                        onClick={() => setExpanded(p => !p)}
                        className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 font-medium flex items-center gap-0.5 transition-colors ml-2"
                    >
                        {expanded ? "Hide" : "Show"} <ChevronRight size={13} className={`transition-transform ${expanded ? "rotate-90" : ""}`} />
                    </button>
                </div>
            </div>

            {/* Collapsible Spotify embed */}
            {expanded && (
                <div className="p-4">
                    <iframe
                        title="Team Worship Playlist"
