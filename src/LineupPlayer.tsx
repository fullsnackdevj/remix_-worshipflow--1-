...
export default function LineupPlayer({ tracks, currentUser, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* ══ MINI BAR — Spotify-style (only shown in mini mode) ══════════════ */}
      {mini && (
        <div
          ref={miniShellRef}
        ...
        >
          ...
          <button onClick={() => setMini(false)} title="Expand player"
      ...
      {/* ══ FULL PLAYER CHROME — header, controls, tracklist (not playerDivId) ══ */}
      {!mini && (
        <div className="fixed z-[9999] bg-gray-900 shadow-2xl rounded-2xl overflow-hidden flex flex-col"
          ...
          {/* Body — video slot is a PLACEHOLDER here; actual iframe is below */}
          <div ref={containerRef} className="flex min-h-0 flex-col md:flex-row" style={{ maxHeight: "calc(90vh - 53px)" }}>
            <div className="flex flex-col flex-shrink-0" style={{ flex: "1 1 0" }}>
              {/* Placeholder that matches the iframe's 16:9 aspect ratio */}
              <div className="relative w-full bg-black" style={{ paddingBottom: "56.25%" }}>
                {/* playerDiv is rendered unconditionally below; this space is for layout */}
              </div>
      ...
      {/* ══ YT PLAYER DIV — THE ONE TRUE HOME ══════════════════════════════════
          NO conditional wrapper. Always in DOM at the same React tree position.
          In full mode: absolutely fills the video slot area via fixed positioning.
          In mini mode: parked off-screen, audio keeps playing.                 */}
      <div
        style={mini
          ? { position: "fixed", left: -9999, top: -9999, width: 1, height: 1, overflow: "hidden", zIndex: 1 }
          : { position: "fixed", zIndex: 9999, pointerEvents: "none",
              // Mirror the video slot: centred modal, below header (53px), 16:9
              top: "calc(50% - 45vh + 53px)",
              left: "50%",
              transform: "translateX(-50%)",
              width: "min(calc(95vw - min(256px, 28.8vw)), calc((90vh - 53px) * 16/9))",
              height: "calc(width * 9/16)",
            }
        }
      >
        <div id={playerDivId} style={{ width: "100%", height: "100%", aspectRatio: "16/9" }} />
      </div>
    </>
  );
}
