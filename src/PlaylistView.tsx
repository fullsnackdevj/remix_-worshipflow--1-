...
export default function PlaylistView({ allSongs, showToast, onNavigateToSongs }: Props) {
  ...
  const [mobileShowList, setMobileShowList]     = useState(true);  // always start on list
  ...
  // ── Pull in global player context ──────────────────────────────────────────
  const {
    playlists: ctxPlaylists, setPlaylists: ctxSetPlaylists,
    ...
    playerContainerId,
    playerRef,
    playSong, togglePlay, handleNext, handlePrev, handleSeek,
    cycleRepeat, doShuffle,
    activePlaylist, playlistSongs, hasSongs, currentIdx,
  } = usePlaylistPlayer();

  // Keep local playlists in sync with context
  useEffect(() => { ctxSetPlaylists(playlists); }, [playlists, ctxSetPlaylists]);

  // ── Desktop video panel: move global YT player div into this view ──────────
  const desktopVideoPanelRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const container = document.getElementById(playerContainerId);
    if (!container || !desktopVideoPanelRef.current) return;
    const panel = desktopVideoPanelRef.current;
    // Make visible + full-size inside the panel
    ...
    return () => {
      // Restore to hidden fixed position when leaving
  ...
  // ── Auto-emit leave event when navigating away ─────────────────────────────
  useEffect(() => {
    return () => { window.dispatchEvent(new CustomEvent("wf-playlist-leave")); };
  }, []);
  ...
  const videoId = currentSong?.video_url ? extractYoutubeId(currentSong.video_url) : null;
    ...
    <div className="flex flex-col h-full w-full bg-[#0d0f14] text-white overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="hidden lg:flex flex-col w-[340px] xl:w-[400px] shrink-0 overflow-hidden bg-[#0a0c10]">
          ...
          {/* Video — context player div moved here */}
          <div className="shrink-0 bg-black w-full aspect-video relative">
            <div ref={desktopVideoPanelRef} className="absolute inset-0 w-full h-full" />
          {!videoId && (
