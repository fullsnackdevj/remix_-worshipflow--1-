...
export default function App() {
  ...
  const handleDeleteSchedule = () => {
    ...
    showConfirm({
      title: "Remove Event",
      message: `Remove "${(editingExisting as any).eventName || "this event"}" from ${editingExisting.date}?`,
      confirmText: "Remove",
      confirmClass: "bg-red-500 hover:bg-red-600",
      onConfirm: async () => {
        try {
          await fetch(`/api/schedules/${editingExisting.id}`, { method: "DELETE" });
          setAllSchedules(prev => prev.filter(s => s.id !== editingExisting.id));
          const remaining = allSchedules.filter(s => s.date === selectedScheduleDate && s.id !== editingExisting.id);
        ...
        } catch { showToast("error", "Could not remove event."); }
      }
    });
  };
                          ...
                          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                            ...
                            <div className="grid grid-cols-7">
                              {Array.from({ length: firstDow }).map((_, i) => (
                                <div key={`e${i}`} className="min-h-[70px] border-b border-r border-gray-50 dark:border-gray-700/50" />
                              ))}
                              {Array.from({ length: daysInMonth }).map((_, i) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`group relative min-h-[70px] border-b border-r border-gray-50 dark:border-gray-700/50 p-1.5 text-left transition-colors ${isCellPast && !cellHasEvents ? "opacity-40 cursor-not-allowed" : "hover:bg-indigo-50 dark:hover:bg-indigo-900/20"} ${isSelected ? "bg-indigo-50 dark:bg-indigo-900/30" : ""}`}
                                  >
                                    ...
                                    {schedEvents.length > 0 && (
                                      <div className="flex flex-col gap-0.5">
                                        <div className="flex items-center gap-0.5 flex-wrap">
                                          {schedEvents.slice(0, 3).map((ev, ei) => {
                                            const nm = (ev as any).eventName || (ev.serviceType === "sunday" ? "Sunday Service" : "Midweek Service");
                                            const dot = nm.toLowerCase().includes("sunday") ? "bg-indigo-500" : nm.toLowerCase().includes("midweek") ? "bg-amber-500" : "bg-emerald-500";
                                            return <span key={ei} className={`w-1.5 h-1.5 rounded-full ${dot}`} />;
                                          })}
                                        </div>
                                        {schedEvents.slice(0, 1).map((ev, ei) => {
                                          const nm = (ev as any).eventName || (ev.serviceType === "sunday" ? "Sunday Service" : "Midweek Service");
                                          return <p key={ei} className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium truncate leading-tight">{nm}</p>;
                                        })}
                                        {schedEvents.length > 1 && <p className="text-[10px] text-gray-400">+{schedEvents.length - 1} more</p>}
                                      </div>
                                    )}
                                  </button>
                                );
                            ...
                            {monthSchedules.length === 0 ? (
                              ...
                              <div className="divide-y divide-gray-100 dark:divide-gray-700">
                                {monthSchedules.sort((a, b) => a.date.localeCompare(b.date)).map(s => {
                                  ...
                                  return (
                                    <div key={s.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer" onClick={() => openEventById(s.id, s.date)}>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                {editingExisting && (
                                  <button onClick={() => {
                                    ...
                                    lines.push("");
                                    if (isServiceEvtCopy) {
                                      lines.push(`*Worship Leader: ${editSchedWorshipLeader?.name ?? "(none)"}`);
                                      if (editSchedBackupSingers.length > 0) lines.push(`*Backup Singers: ${editSchedBackupSingers.map(b => b.name).join(", ")}`);
                                      if (editSchedMusicians.length > 0) lines.push(`*Musicians / Instruments: ${editSchedMusicians.map(m => `${m.name} (${m.role})`).join(", ")}`);
                                      ...
                                      if (sSong) { lines.push(`Solemn: ${sSong.title}${sSong.artist ? ` - ${sSong.artist}` : ""}`); if (sSong.video_url) lines.push(`Link: ${sSong.video_url}`); }
                                    } else {
                                      editSchedAssignments.forEach(asgn => { lines.push(`*${asgn.role}: ${asgn.members.map(m => m.name).join(", ") || "(none)"}`); });
                                      if (editSchedAssignments.length > 0) lines.push("");
                                    }
                                    if (editSchedNotes) { lines.push("*Notes:"); lines.push(editSchedNotes); }
                            ...
                            {schedPanelMode === "view" && editingExisting ? (
                              ...
                              <div className="space-y-4">
                                {isServiceEvent && (
                                  <>
                                    <div>
                                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Worship Leader</p>
                                      {editSchedWorshipLeader
                                        ? <div className="flex items-center gap-2">
                                          {editSchedWorshipLeader.photo ? <img src={editSchedWorshipLeader.photo} className="w-8 h-8 rounded-full object-cover" alt="" /> : <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white text-xs font-bold">{editSchedWorshipLeader.name[0]}</div>}
                                          <span className="font-semibold text-sm text-gray-900 dark:text-white">{editSchedWorshipLeader.name}</span>
                                        </div>
                                        : <span className="text-sm text-gray-400 italic">Not set</span>}
                                    </div>
                                    {editSchedBackupSingers.length > 0 && (
                                      <div>
                                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Backup Singers</p>
                                        <div className="flex flex-col gap-1">
                                          {editSchedBackupSingers.map((m, i) => <div key={i} className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-purple-500 flex items-center justify-center text-white text-[10px] font-bold">{m.name[0]}</div><span className="text-sm">{m.name}</span></div>)}
                                        </div>
                                      </div>
                                    )}
                                    {editSchedMusicians.length > 0 && (
                                      <div>
                                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Musicians</p>
                                        <div className="flex flex-col gap-1">
                                          {editSchedMusicians.map((m, i) => <div key={i} className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center text-white text-[10px] font-bold">{m.name[0]}</div><span className="text-sm">{m.name}</span><span className="text-xs text-gray-400">{m.role}</span></div>)}
                                        </div>
                                      </div>
                                    )}
                                    {(editSchedSongLineup.joyful || editSchedSongLineup.solemn) && (
                                      <div>
                                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Song Line-up</p>
                                        {editSchedSongLineup.joyful && <p className="text-sm">🌟 <span className="font-medium text-amber-600 dark:text-amber-400 uppercase text-xs">Joyful</span> {allSongs.find(sg => sg.id === editSchedSongLineup.joyful)?.title ?? "—"}</p>}
                                        {editSchedSongLineup.solemn && <p className="text-sm">🙏 <span className="font-medium text-indigo-500 uppercase text-xs">Solemn</span> {allSongs.find(sg => sg.id === editSchedSongLineup.solemn)?.title ?? "—"}</p>}
                                      </div>
                                    )}
                                  </>
                                )}
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                          {asgn.members.map((m, mi) => (
                                            <div key={mi} className="flex items-center gap-2">
                                              {m.photo
                                                ? <img src={m.photo} className="w-6 h-6 rounded-full object-cover" alt="" />
                                                : <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-[10px] font-bold">{m.name[0]}</div>
                                              }
                                      ...
                                      <input
                                        ...
                                        onKeyDown={e => {
                                          if (e.key === "Enter" && newRoleInput.trim()) {
                                            setEditSchedAssignments(prev => [...prev, { role: newRoleInput.trim(), members: [], search: "" }]);
                                            setNewRoleInput("");
                                          }
                                        }}
                                  ...
                                  return (
                                    <>
                                      ...
                                      <div>
                                        ...
                                        <div className="space-y-0.5 max-h-44 overflow-y-auto pr-1">
                                          {muCandidates.map(m => {
                                            const memberRoles = ((m as any).roles || []).join(", ") || "Musician";
                                            return (
                                              <button key={m.id} type="button"
                                                onClick={() => setEditSchedMusicians(prev => [...prev, { memberId: m.id, name: m.name, photo: m.photo, role: memberRoles }])}
                                                className="w-full flex items-center gap-3 px-2.5 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/60 transition-colors">
                                                {m.photo
                                                  ? <img src={m.photo} className="w-9 h-9 rounded-full object-cover shrink-0" alt="" />
